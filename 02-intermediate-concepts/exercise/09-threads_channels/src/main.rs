...
fn main() {
   ...
   let handle_b = thread::spawn(move || {
      sleep_ms(0);
      tx1.send("Thread B: 1").unwrap();
      sleep_ms(200);
      tx1.send("Thread B: 2").unwrap();
   });
   ...
   for msg in vec!["ab", "bc", "cd", "de", "ef", "fg"] {
      println!("Main thread: Processing the letter '{}'", msg);
      tx3.send(msg).unwrap();
      sleep_ms(100);
      tx4.send(msg).unwrap();
      sleep_ms(100);
   ...
   let result_c = handle_c.join();
   let result_d = handle_d.join();
